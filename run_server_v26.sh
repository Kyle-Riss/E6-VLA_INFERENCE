...
#  vision LoRA: layers 22~25 (Late 상위, 미완료 7500step)
#  norm_stats : 자동 로딩 (assets/Kyle-Riss/dobot_e6_pick_place_orange_v16/)
#
#  사용법:
#    bash run_server_v26.sh [체크포인트경로]
# ══════════════════════════════════════════════════════════════════════════════

set -e

REPO="$(cd "$(dirname "$0")" && pwd)"
DEFAULT_CKPT="/media/billye6/새 볼륨1/e6_checkpoints/e6_v26_7500"
...
python "$REPO/scripts/serve_policy.py" \
    --port 8000 \
    policy:checkpoint \
    --policy.config pi05_e6_v26_lora \
