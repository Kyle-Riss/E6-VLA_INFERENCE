...
#  vision LoRA: layers 14~25 (v16보다 넓은 범위)
#  norm_stats : 자동 로딩 (assets/Kyle-Riss/dobot_e6_pick_place_orange_v16/)
#
#  사용법:
#    bash run_server_v17.sh [체크포인트경로]
# ══════════════════════════════════════════════════════════════════════════════

set -e

REPO="$(cd "$(dirname "$0")" && pwd)"
DEFAULT_CKPT="/media/billye6/새 볼륨1/e6_checkpoints/e6_v17_15000"
...
python "$REPO/scripts/serve_policy.py" \
    --port 8000 \
    policy:checkpoint \
    --policy.config pi05_e6_v17_lora \
