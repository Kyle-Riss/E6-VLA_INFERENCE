...
# DOBOT_DEBUG=1 이면 sendRecvMsg/응답 등 상세 로그 출력 (기본: 끔)
DOBOT_DEBUG = os.environ.get("DOBOT_DEBUG", "").strip() in ("1", "true", "yes")
    ...
    def __init__(self, ip, port, *args):
        ...
        if self.port == 29999 or self.port == 30004 or self.port == 30005:
            try:
                ...
                print(f"Dobot 소켓 연결 실패 ({self.ip}:{self.port}): {e}")
                raise
...
class DobotApiMove(DobotApiDashboard):
    """
    Control and motion command interface (Wrapper)
