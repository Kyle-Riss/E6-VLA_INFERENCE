...
시스템 python3(matplotlib 설치됨)으로 실행. (move-one venv 엔 matplotlib 없음)

smolvla 의 plot_infer_resource.py 와 동일한 형식:
...
def main():
    ...
    title = (f"π0.5 (e6) inference resource usage  "
             f"({meta.get('label', '')}, mean of {meta.get('runs')} runs)")
